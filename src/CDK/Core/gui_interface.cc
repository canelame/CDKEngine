...
void GuInterface::draw(std::shared_ptr<Node>node){
  ...
  ImGui::Begin("Shadow panel",0,0);
  ImGui::SetWindowSize(ImVec2(200, 300));
  ImGui::SetWindowPos(ImVec2(300, 300));

  if (ImGui::Checkbox("Activate shadow", &active_shadow_)){
      EngineManager::instance().enable_shadows_ = (int)active_shadow_;
  }
  ImGui::End();
...
void GuInterface::transformPanel(Node &node){
    ...
    if (ImGui::InputFloat3("Position:", n_pos)){
    ...
    if (ImGui::InputFloat3("Scale:", n_scale)){
    ...
    if (ImGui::InputFloat3("Rotation:",n_rotate)){
