...
void DrawCommand::runCommand()const{
  ...
  OpenGlInterFaz::instance().drawGeometry(t_geo->getVAO(),t_geo->getBufferIndex(), t_geo->indiceSize());
...
void SendObjectShadow::runCommand()const{
  ...
  OpenGlInterFaz::instance().drawGeometry(t_geo->getVAO(),t_geo->getBufferIndex() ,(unsigned int)t_geo->indiceSize());
}
