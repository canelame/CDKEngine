...
void Composer::render(){
  ...
  for (int i = 0; i < effects_list_.size(); i++){
    ...
    if (fb->isLoaded() == false){
      OpenGlInterFaz::instance().createFrameBuffer(*fb);
    }
    ...
    OpenGlInterFaz::instance().drawGeometry((unsigned int)buff->getVAO(),buff->getBufferIndex(), 6);
  }
